...
const OperationReviewSaveReply = "/api.review.v1.Review/SaveReply"
...
type ReviewHTTPServer interface {
	...
	// SaveReply 保存评价回复
	SaveReply(context.Context, *SaveReplyRequest) (*SaveReplyReply, error)
...
func RegisterReviewHTTPServer(s *http.Server, srv ReviewHTTPServer) {
	...
	r.POST("/v1/review/{reviewID}/reply", _Review_SaveReply0_HTTP_Handler(srv))
	r.PUT("/v1/review/{reviewID}/audit", _Review_AuditReview0_HTTP_Handler(srv))
	r.POST("/v1/review/{reviewID}/appeal", _Review_AppealReview0_HTTP_Handler(srv))
	r.PUT("/v1/appeal/{appealID}/audit", _Review_AuditAppeal0_HTTP_Handler(srv))
}
...
func _Review_SaveReply0_HTTP_Handler(srv ReviewHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in SaveReplyRequest
		if err := ctx.Bind(&in); err != nil {
			return err
		}
		if err := ctx.BindQuery(&in); err != nil {
		...
		http.SetOperation(ctx, OperationReviewSaveReply)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.SaveReply(ctx, req.(*SaveReplyRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*SaveReplyReply)
		return ctx.Result(200, reply)
	}
}
...
type ReviewHTTPClient interface {
	...
	SaveReply(ctx context.Context, req *SaveReplyRequest, opts ...http.CallOption) (rsp *SaveReplyReply, err error)
...
func (c *ReviewHTTPClientImpl) AppealReview(ctx context.Context, in *AppealReviewRequest, opts ...http.CallOption) (*AppealReviewReply, error) {
	...
	pattern := "/v1/review/{reviewID}/appeal"
...
func (c *ReviewHTTPClientImpl) AuditAppeal(ctx context.Context, in *AuditAppealRequest, opts ...http.CallOption) (*AuditAppealReply, error) {
	...
	pattern := "/v1/appeal/{appealID}/audit"
	path := binding.EncodeURL(pattern, in, false)
...
func (c *ReviewHTTPClientImpl) AuditReview(ctx context.Context, in *AuditReviewRequest, opts ...http.CallOption) (*AuditReviewReply, error) {
	...
	pattern := "/v1/review/{reviewID}/audit"
	path := binding.EncodeURL(pattern, in, false)
...
func (c *ReviewHTTPClientImpl) SaveReply(ctx context.Context, in *SaveReplyRequest, opts ...http.CallOption) (*SaveReplyReply, error) {
	var out SaveReplyReply
	pattern := "/v1/review/{reviewID}/reply"
	path := binding.EncodeURL(pattern, in, false)
	opts = append(opts, http.Operation(OperationReviewSaveReply))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "POST", path, in, &out, opts...)
	if err != nil {
		return nil, err
